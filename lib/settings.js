...
class Settings extends EventEmitter {
  ...
  _tryReadAndParseJsonFile(filePath) {
    ...
    if (fs.existsSync(filePath)) {
      try {
        response.content = fs.readFileSync(filePath, 'utf8');
        response.jsonObject = JSON.parse(response.content);
        response.isOk = true;
      } catch (e) {
        winston.error(e);
        response.isOk = false; // not json format => need to write default setting
        this.emit(Settings.Events.ERROR, {
          error: e,
  ...
  _readSettings() {
    ...
    if (parseJsonResult.isOk) {
      return parseJsonResult.jsonObject;
    }
