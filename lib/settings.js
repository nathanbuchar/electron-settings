...
class Settings extends EventEmitter {
  ...
  _buildWatchPaths(path) {
    const children = path.split('.');
    const paths = [];

    for (let i = 0; i < children.length; i++) {
      const path = _.take(children, i + 1);
      paths.push(_.join(path, '.'));
    }
  ...
  _handleObjectChanged(keyPath = '') {
    ...
    let relatedHandler = [];
    if (keyPath) {
      const paths = this._buildWatchPaths(keyPath);
      relatedHandler =
      _.filter(this._watchHandlers, f => _.some(paths, c => f.keyPath.startsWith(c)));
    } else {
      relatedHandler = this._watchHandlers;
    }
  ...
  watch(keyPath, handler) {
    ...
    const observer =
    new Observer(this, keyPath, handler, _.cloneDeep(_.get(this._cacheObj, keyPath)));
