...
/**
 * Empties all the set configurations. Sugar for `ElectronSettings.unset('.')`.
 *
 * @access public
 * @returns {ElectronSettings} this
 */
internals.ElectronSettings.prototype.empty = function (options) {
  debug('emptying settings');
...
/**
 * Empties all the set configurations and replaces them with the new desired set
 * of config.
 *
 ...
 * @access public
 */
internals.ElectronSettings.prototype.reset = function (defaults, options) {
  debug('resetting settings');

  options = this._parseOptions(options);

  this.empty().set('.', defaults);
