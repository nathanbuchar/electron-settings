...
class Settings extends Watcher {
  ...
  _setupInternalCache() {
    ...
    try {
      this._settingsCache = JSON.parse(contents, this._deserialize);
    } catch (err) {
  ...
  _save() {
    ...
    fs.writeFile(this._configFilePath, JSON.stringify(this._settingsCache, this._serialize, 4), err => {
      if (err) {
  ...
  /**
   * Serializes object so that we can store RegExp types
   * 
   * @param  {String} k Key of this JSON object
   * @param  {Object} v Value of this JSON object
   * @return {Object}   Altered or original value
   */
  _serialize(k, v) {
    if (util.isRegExp(v)) {
  ...
  /**
   * Deserializes object so that we can restore RegExp types
   * 
   * @param  {String} k Key of this JSON object
   * @param  {Object} v Value of this JSON object
   * @return {Object}   Altered or original value
   */
  _deserialize(k, v) {
    if (typeof v !== 'string' || v.indexOf('$regex$') === -1) {
      return v
    }
    var r = v.split('$regex$')[1];
    var spl = r.split('/');
    var flags = spl.pop();
  ...
  _getConfigDirPath() {
    try {
      ...
      let app = !!remote ? remote.app : (require('electron').app || require('app'));
