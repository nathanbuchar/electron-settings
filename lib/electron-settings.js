...
class ElectronSettings extends EventEmitter {
   ...
   * @param {string} [options.file=settings.json]
  ...
  /**
   * Sets up the instances options by extending the default ElectronSettings
  ...
  _initConfigFilePath() {
    this._configFilePath = path.join(
      this._options.dir,
      this._options.file.split('.json')[0] + '.json'
    );
  }
   ...
   */
  _initSettings() {
    let data = this._options.defaults;

    try {
      data = this._readSettingsFileSync();
    } catch (err) {
      this._outputSettingsFile(data);
    } finally {
      this._cache = data;
    }
  }
  ...
  _outputSettingsFile(obj, fn) {
    fs.outputJson(this._configFilePath, obj, fn);
  }
  ...
  */
  ...
  _deferredSave(opts) {
    return new Promise((resolve, reject) => {
      if (!opts.shouldSave) return resolve();
  ...
  defaults(obj, options={}) {
    const opts = Object.assign({
      shouldSave: ElectronSettings.DefaultOptions.SHOULD_SAVE,
      saveImmediately: ElectronSettings.DefaultOptions.SAVE_IMMEDIATELY,
      overwrite: ElectronSettings.DefaultOptions.OVERWRITE
    }, options);
